...
def checkusr(event=None):
    ...
    if users.has_key(usr.get()) == True and ch() == True:
        ...
        def pwdfromlbl():
            ...
            print buttonchoice.get()
            p = tk.Label(window, text = ' ' + pwds[buttonchoice.get()], font = ('bold', 12))
        ...
        for file in results:
            if file.startswith(encrypt(usrname, 11)):
                f = file
                sellbl.config(text = 'You are logged in as: ' + usrname + '\nThe SecureLocker File that is associated with your account is: ' +  file)
        ...
        print results
        lbls = declabelandpwd(file, 'labels')
        lblsd = dict(enumerate(lbls))
        for ind, lbl in lblsd.items(): 
                print ind
                tk.Radiobutton(window, indicatoron = 0, width = 15, text = lbl, variable = buttonchoice, value = ind , command=pwdfromlbl).grid(row = r, column =  1)
                r += 1
        e = tk.Button(window, text = 'ENCRYPT \nPASSWORD', width = 15, pady = 5, command = encpass)
        d = tk.Button(window, text = 'SHOW DECRYPTED \nPASSWORD', width = 15, pady = 5)
